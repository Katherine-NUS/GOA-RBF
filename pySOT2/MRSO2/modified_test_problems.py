...
class Shift_Rastrigin:
    ...
    def __init__(self, dim=10):
        ...
        self.dim = dim
        self.xo=np.ones(dim)
        self.info = str(dim)+"-dimensional Shift_Rastrigin function \n" +\
                             "Global optimum: f(0,0,...,0) = 0"
        self.min = 0
        self.integer = []
        self.continuous = np.arange(0, dim)
        ...
        for k in range(dim):
            if np.mod(k,2)==0:
                self.xo[k]= self.xlow[k]
            else:
                self.xo[k] = self.xup[k]
    ...
    def objfunction(self, x):
        ...
        if len(x) != self.dim:
            raise ValueError('Dimension mismatch')
        return 10 * self.dim + sum((x-self.xo)**2 - 10 * np.cos(2 * np.pi * (x-self.xo)))
...
class Shift_Ackley:
    ...
    def __init__(self, dim=10):
        ...
        self.dim = dim
        self.xo = np.ones(dim)
        self.info = str(dim)+"-dimensional Shift_Ackley function \n" +\
                             "Global optimum: f(0,0,...,0) = 0"
        self.min = 0
        self.integer = []
        self.continuous = np.arange(0, dim)
    ...
    def objfunction(self, x):
        ...
        if len(x) != self.dim:
            raise ValueError('Dimension mismatch')
        n = float(len(x))
        return -20.0 * np.exp(-0.2*np.sqrt(np.sum((x-self.xo)**2)/n)) - \
            np.exp(np.sum(np.cos(2.0*np.pi*(x-self.xo)))/n) + 20 + np.exp(1)
...
class Michalewicz:
    ...
    def objfunction(self, x):
        ...
        return -np.sum(np.sin(x) * (np.sin(((1+np.arange(self.dim))
                                            * x**2)/np.pi)) ** 20)
class Shift_Michalewicz:
    """Shift_Michalewicz function
    """
    def __init__(self, dim=10):
        self.xlow = np.zeros(dim)
        self.xup = np.pi * np.ones(dim)
        self.dim = dim
        self.xo = np.ones(dim)
        self.info = str(dim)+"-dimensional Shift_Michalewicz function \n" +\
                             "Global optimum: ??"
        self.min = np.NaN
        self.integer = []
        self.continuous = np.arange(0, dim)
    ...
    def objfunction(self, x):
        ...
        if len(x) != self.dim:
            raise ValueError('Dimension mismatch')
        return -np.sum(np.sin((x-self.xo)) * (np.sin(((1+np.arange(self.dim))
                                            * (x-self.xo)**2)/np.pi)) ** 20)
...
class Shift_Levy:
    ...
    def __init__(self, dim=10):
        ...
        self.min = 0.0
        self.xo = np.ones(dim)
        self.info = str(dim)+"-dimensional Shift_Levy function \n" +\
                             "Global optimum: ?"
        self.integer = []
        self.continuous = np.arange(0, dim)
        check_opt_prob(self)
    ...
    def objfunction(self, x):
        ...
        b = 0.2
        d = 50
        return -a * np.exp(-b * np.sqrt(np.sum((x-self.xo) ** 2)/d)) -\
            np.exp(np.sum(np.cos(2 * np.pi * (x-self.xo)))/d)
...
class Shift_Rosenbrock:
    """Shift_Rosenbrock function
    """
...
class Shift_Whitley:
    ...
    def __init__(self, dim=10):
        ...
        self.dim = dim
        self.xo = np.ones(dim)
        self.info = str(dim)+"-dimensional Shift_Whitley function \n" +\
                             "Global optimum: f(1,1,...,1) = 0"
        self.min = 0
        self.integer = []
        self.continuous = np.arange(0, dim)
    ...
    def objfunction(self, x):
        ...
        for i in range(len(z)):
            temp = 100*((np.repeat(z[i]**2,len(z)))-z) + (1-z)**2
            total+=np.sum((float(item**2)/4000.0) - np.cos(item) + 1 for item in temp)
...
class Whitley:
    ...
    def objfunction(self, x):
        ...
        total = 0
        for i in range(len(x)):
            temp = 100*((np.repeat(x[i]**2,len(x)))-x) + (1-x)**2
            total+=np.sum((float(item**2)/4000.0) - np.cos(item) + 1 for item in temp)
...
class Shift_SchafferF7:
    """Shift_SchafferF7 function
    """
    ...
    def __init__(self, dim=10):
        ...
        self.dim = dim
        self.xo = np.ones(dim)
        self.info = str(dim)+"-dimensional Shift_SchafferF7 function \n" +\
                             "Global optimum: f(0,0,...,0) = 0"
        self.min = 0
        self.integer = []
        self.continuous = np.arange(0, dim)
...
class Schoen20k:
    ...
    def __init__(self, dim=10,f=[],z = []):
        self.xlow =  np.zeros(dim)
        self.xup = 1.0* np.ones(dim)
        self.dim = dim
        self.f = f
        self.z = z
        self.info = "20-k-"+str(dim)+"-dimensional Schoen function \n" +\
    ...
    def objfunction(self, x):
        ...
        k = len(StatValue)
        Xtemp = np.dot(np.ones((k,1)),np.atleast_2d(x))
        TempNorm = np.sum(np.power(Xtemp-StatPoint,2),1)
        TempProd = np.ones((1,k))
        for i in range(1,k+1):
            TempProd[0,i-1] = np.prod(TempNorm[0:i-1])*np.prod(TempNorm[i:k])
        return float((np.dot(TempProd,StatValue))/np.sum(TempProd))
...
class Schoen100k:
    ...
    def __init__(self, dim=10,f=[],z = []):
        ...
        self.f = f
        self.z = z
        self.info = "100-k-"+str(dim)+"-dimensional Schoen function \n" +\
                             "Global optimum: unknown"
        self.min = 0
        self.integer = []
        self.continuous = np.arange(0, dim)
...
class Dixonprice:
    ...
    def __init__(self, dim=2):
        ...
        self.dim = dim
        self.info = str(dim)+"-dimensional Dixonprice function \n"
        self.integer = []
        self.continuous = np.arange(0, dim)
