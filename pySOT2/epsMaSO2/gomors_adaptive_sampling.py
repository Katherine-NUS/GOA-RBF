...
class EvolutionaryAlgorithm(object):
    ...
    def __init__(self, data, epsilons, numcand=None, cand_flag=None):
        ...
        if self.numcand is None:
            self.numcand = min([500, 50*data.dim])
        self.budget = None
        self.epsilons = epsilons

        self.select_rules = MultiRuleSelection(self.data, self.epsilons)
    ...
    def __generate_cand__(self, xfront, xgap = None):
        #pop_size = max([50, min([100, 2 * len(xfront)])])
        pop_size = 200

        if xgap is None:
            problem = GlobalProblem(self.data.dim, self.data.nobj, [fhat for fhat in self.fhat])
            generator = CustomGenerator(pop_size)
            if np.random.rand() < 1.0:
                generator.create(problem, None, None)
            else:
                generator.create(problem, xfront, None)

            # algorithm = NSGAIII(problem, divisions_outer=100, generator=generator)
            # algorithm = NSGAII(problem=problem, population_size=pop_size, generator=generator)
            # algorithm = EpsNSGAII(problem=problem, epsilons=self.epsilons, population_size=pop_size, generator=generator)
            algorithm = EpsMOEA(problem=problem, epsilons=np.asarray([0.01, 0.01]), population_size=pop_size, generator=generator)
            algorithm.run(2000)
        else:
            rgap = 0.1
            problem = GapProblem(self.data.dim, self.data.nobj, [fhat.types[fhat.current_type] for fhat in self.fhat],  xgap, rgap)
    ...
    def make_points(self, npts, xbest, xfront, front, ac_xfront, ac_front, subset=None, proj_fun=None):
        """Proposes npts new points to evaluate
        ...
        index = rule1.select_points(xcand_nd=np.copy(self.xcand_nd),
                                    fhvals_nd=np.copy(self.fhvals_nd),
                                    front=np.copy(front), proposed_points=np.copy(self.previous_points),
                                    indices=index, rate = 0.9)
        ...
        totalTime = end - start
        print('Candidate Generation and Selection Time: ' + str(totalTime))


        index = self.select_rules.select_points(xcand_nd = np.copy(self.xcand_nd),
                                                fhvals_nd = np.copy(self.fhvals_nd),
                                                front = np.copy(front), proposed_points = np.copy(self.previous_points),
                                                indices = index, npts = npts)
